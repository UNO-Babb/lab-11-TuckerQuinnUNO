...
def shopper(env, id):
        arrive = env.now
        items = random.randint(0,55)
        if items > 25:     # Random, but pushing slightly towards a bell curve shape
            items = items - 5
        else: 
            items = items + 5
        shoppingTime = items // (random.randint(1,3)) #shopping takes 1 to 1/3 of a minute 
...
def customerArrival(env):
    ...
    while True:
        ...
        yield env.timeout(random.randint(1,5)) #New shopper every 1 to 5 minutes
